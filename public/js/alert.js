class Alert{
    ...
    beforeSingUp(button, form){
        ...
        $(document).on('click', button, function (e) {
            e.preventDefault()
            const array = [];
            that.alertSingIn('#firstName', 4, array);
            that.alertSingIn('#lastName', 4, array);
            that.phoneVerify('#phone', array);
            that.emailVerify('#email', array);
            that.alertSingIn('#password', 8, array);
    ...
    beforeUpdateUser(button, form){
        ...
        $(document).on('click',button, function (e) {
            e.preventDefault();
            const array = [];
            that.alertSingIn('#updateFirstName', 4, array);
            that.alertSingIn('#updateLastName', 4, array);
    ...
    beforeCreateAdmin(button, form){
        ...
        $(document).on('click',button, function (e) {
            e.preventDefault()
            const array = [];
            that.alertSingIn('#adminFirstName', 4, array);
            that.alertSingIn('#adminLastName', 4, array);
